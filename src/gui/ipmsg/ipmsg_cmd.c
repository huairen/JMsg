...
int handle_user_entry(struct msg_packet *packet)
{
	...
	if(user) {
		...
		user->status = packet->command;
	}

	msg_send(packet->addr.ip, IPMSG_ANSENTRY, get_nick_name(), cfg_group_name());
...
{
	uint32_t user_id = hashlittle(packet->host_name, strlen(packet->host_name), 0);
	user_remove(user_id);
	return user_id;
}
